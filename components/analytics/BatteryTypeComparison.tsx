...
export function BatteryTypeComparison({ vehicles, isLoading }: Props) {
  ...
  const data = useMemo(() => {
    ...
    for (const v of vehicles) {
      ...
      groups[v.battery_type].push(calculateHealthScore(v));
    }
    return Object.entries(groups).map(([batteryType, scores]) => ({
      batteryType,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Health by Battery Type</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              <XAxis dataKey="batteryType" stroke={colors.fgFaint} tick={{ fontSize: 12 }} />
              ...
              />
              <Bar dataKey="avgHealth" fill={colors.primary} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
